...
class MasterModel extends Model{
	...
	public function getAllPostTag() {
        $sql="select * from page_location where deleted_at=0"; 
		$query=$this->db->query($sql);
		$result=$query->getResultArray();
		return $result;
    }
	public function getSinglePostTag($id = false) {
        $sql="select * from page_location where id='".$id."'"; 
		$query=$this->db->query($sql);
		$result=$query->getResultArray();
		if(isset($result[0])){
		return $result[0];	
	...
	public function deletePostTag($id){
		$sql="update page_location set deleted_at=1 where id='".$id."'"; 
		$query=$this->db->query($sql);
	}
	public function insertPostTag($data){
		$this->db->table('pagge_location')->insert($data);
	}
	public function updatePostTag($id,$data){
		$sql="update page_location set name='".$data['name']."' where id='".$id."'"; 
		$query=$this->db->query($sql);
	...
	public function getPostComment($id = false) {
       $sql="select fpco.id,fpco.admin_id,fpco.comment_id,fpco.user_id,fpco.message,fpco.created_at,u.name as users_name,u.profile_image as users_profile_image,u.email as users_email,a.name as admin_name,a.email as admin_email from forum_post_comments fpco 
		left join users u
