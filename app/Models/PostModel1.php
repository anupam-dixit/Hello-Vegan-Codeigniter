...
class PostModel1 extends Model{
	...
	public function getSingleUserPost($id) {
		$result=array();
		if($id!=''){
         $sql="Select * from posts where posted_by='".$id."' order by id desc"; 
	...
	public function getVeganPostLimit($id){
		$sql="select p.*,(select count(liked_by) from post_like  where post_id=p.id) as total_like,(select count(message) from post_comments  where post_id=p.id) as total_comment,p.created_at as post_created_at,p.content as post_content,p.image as post_image,us.id as user_id,us.name as user_name,us.profile_image as user_image,(select count(status) from post_like where post_id=p.id and liked_by='".$id."') as likestatus from posts p inner join users us on us.id=p.posted_by where  p.deleted_at=0  order by p.id desc limit 0,6"; 
	...
	public function getVeganPostLimits($id,$limit, $start){
		$sql="select p.*,(select count(liked_by) from post_like  where post_id=p.id) as total_like,(select count(message) from post_comments  where post_id=p.id) as total_comment,p.created_at as post_created_at,p.content as post_content,p.image as post_image,us.id as user_id,us.name as user_name,us.profile_image as user_image,(select count(status) from post_like where post_id=p.id and liked_by='".$id."') as likestatus from posts p inner join users us on us.id=p.posted_by where  p.deleted_at=0  order by p.id desc limit $start,$limit"; 
	...
	public function getUserPhotos($id) {
		...
		union select profile_image as image from users where id='".$id."' limit 0,8"; 
		$query=$this->db->query($sql);
		$result=$query->getResultArray();
	    $data=[];
	    if(count($result)!=0){
	...
	public function getVeganPostUser($id){
		$sql="select p.*,p.created_at as post_created_at,p.content as post_content,p.image as post_image,us.name as user_name,us.profile_image as user_image,(select count(status) from post_like where post_id=p.id and liked_by='".$id."') as likestatus from posts p inner join users us on us.id=p.posted_by where  p.deleted_at=0 and p.id='".$id."'  order by p.id desc"; 
		$query=$this->db->query($sql);
		$result=$query->getResultArray();
